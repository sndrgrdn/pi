...
export function buildCancellationReport(log: readonly TaskToolLogEntry[]): string {
	...
	for (const entry of log) {
		...
		const label = command ? `Command: ${command.slice(0, 80)}${command.length > 80 ? "…" : ""}` : `${entry.tool}${path ? `: ${path}` : ""}`;
...
export interface TaskDependencies {
	basePrompts?: (cwd: string) => Pick<TaskPromptParts, "system" | "appendSystem" | "projectContext"> | Promise<Pick<TaskPromptParts, "system" | "appendSystem" | "projectContext">>;
}

export function createTaskTool(runner: Pick<SubagentRunner, "run">, profiles: ResolvedProfiles, dependencies: TaskDependencies = {}): ToolDefinition<any, any, any> {
	return {
		name: "task", label: "task",
		description: "Delegate a bounded mutation. Specify verification steps; summarize the returned report for the user.",
		parameters: Type.Object({
			prompt: Type.String({ description: "Complete implementation brief, including constraints and verification steps." }),
			description: Type.String({ description: "Short TUI description of the delegated work." }),
			mode: Type.Optional(Type.Union([Type.Literal("low"), Type.Literal("medium"), Type.Literal("high")], { description: "Route: low = Sol/low; medium = Sol/high; high = Fable 5/high. Defaults to low." })),
		}),
		async execute(_id, params: TaskInput, signal, onUpdate, ctx) {
			const mode = params.mode ?? "low";
			onUpdate?.({ content: [{ type: "text", text: "working" }], details: { state: "running", mode, description: params.description } });
			const base = await (dependencies.basePrompts ?? readBasePrompts)(ctx.cwd);
			const skills = discoverChildSkills(ctx.cwd);
			const refs = extractSkillRefs(params.prompt, skills.map((skill) => skill.name));
			...
			try {
				...
				toolbox: (processes) => [
					createShellCommandTool(processes), createShellStatusTool(processes), createShellCancelTool(processes),
					createHarnessReadTool(), createApplyPatchTool(), createChildSkillTool(skills),
					createFinderTool(runner, profiles), createLibrarianTool(runner, profiles),
				],
				...
				if (failure.name === "AbortError") {
					envelope = buildEnvelope({ kind: "task_error", sessionID: failure.sessionID, content: buildCancellationReport(failure.toolLog) });
				} else {
					try {
						const summary = await runner.run({
						definition: resolveAgentDefinition({
							key: "task", allowMcp: false, tools: [],
							systemPrompt: "Summarize a failed Task run from its tool log. Report accomplishments, files modified, findings, verification, and unfinished work. Do not invent facts.",
						}, resolveAgentRoute(profiles, "task", mode)),
						cwd: ctx.cwd,
						...
						if (!(summaryError instanceof SubagentRunError) || summaryError.name !== "AbortError") throw summaryError;
						envelope = buildEnvelope({ kind: "task_error", sessionID: failure.sessionID, content: buildCancellationReport(failure.toolLog) });
					}
				}
			}
			return { content: [{ type: "text", text: envelope }], details: { state: "complete", mode, description: params.description } };
		},
		renderCall(args: TaskInput | undefined, theme, context) { return renderer.renderCall({ detail: args?.description }, theme, context); },
		renderResult(result, options, theme, context) { return renderer.renderResult(result, options, theme, context); },
