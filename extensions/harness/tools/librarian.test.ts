...
describe("librarian tool", () => {
	it("prepends optional context and returns the final answer envelope", async () => {
		...
		const result = await tool.execute("call", { query: "How?", context: "Ruby 3.4" }, undefined, undefined, { cwd: "/repo" } as any);
		expect(result.content[0]).toMatchObject({ type: "text", text: expect.stringContaining('<librarian_result sessionID="library-1">') });
	...
	it("maps context exhaustion to actionable guidance", () => {
		expect(mapLibrarianError(new Error("maximum context length exceeded")).message).toContain("try a more specific query");
