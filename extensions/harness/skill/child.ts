...
export function discoverChildSkills(cwd: string): SkillEntry[] {
	return loadSkills({ cwd, agentDir: getAgentDir(), skillPaths: [], includeDefaults: true }).skills.map(({ name, description, filePath, baseDir }) => ({
...
export function createChildSkillTool(skills: readonly SkillEntry[]): ToolDefinition<any, any, any> {
	const unavailable = (name: string) => availableSkillsBlock(skills.map((entry) => entry.name), name);
	return {
		name: "skill", label: "skill", description: "Load a skill by exact name.",
		...
		async execute(_id: string, params: { name: string }) {
			...
			try {
				...
				throw new Error(`Failed to load skill "${params.name}" from ${skill.filePath}: ${error instanceof Error ? error.message : String(error)}\n${unavailable(params.name)}`);
			}
		},
