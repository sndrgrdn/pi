...
const schema = Type.Object({
	path: Type.String({
		description: [
			"Path to the file to edit. Relative paths resolve against the current working directory; absolute paths are accepted.",
			"The file MUST already exist — `edit` cannot create files. Use `write` for new files.",
			"`edit` cannot delete files either. Use `bash` with `rm` for deletion.",
		].join(" "),
	}),
	edits: Type.Array(
		Type.Object(
			{
				oldText: Type.String({
					description: [
						"Exact substring to replace, taken from the ORIGINAL file (pre-edit, before any other edits in this call are applied).",
						"Constraints:",
						"(1) Must occur EXACTLY ONCE in the file. If not unique, expand it with surrounding lines until it is.",
		...
		),
		{
			...
			description: [
				...
				"Edits must not overlap or be nested. If two changes touch the same block or adjacent lines, merge them into ONE entry whose oldText/newText cover both.",
...
const description = [
	...
	"All oldText values match against the original file content, not intermediate results. Prefer one call with multiple edits over sequential calls.",
