...
def process_payment(order)
  client = Stripe::Client.new(ENV.fetch("STRIPE_KEY"))
...
class ApiClient
  def get_user(id) = get("/users/#{id}")
  def get_orders(user_id) = get("/users/#{user_id}/orders")
  def create_order(data) = post("/orders", data)
end
