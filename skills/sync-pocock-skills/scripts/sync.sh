...
# --- Discover installed skills that exist upstream (implicit tracking) ---
...
for dir in "$SKILLS_DIR"/*/; do
  ...
  name=$(basename "$dir")
  # Only track skills that exist upstream
...
# --- Section 1: New upstream skills (not installed, not ignored) ---
echo ""
echo "=== NEW_SKILLS ==="
while IFS=$'\t' read -r name upstream_path; do
  our_path=$(get_our_path "$name")
  if [[ -z "$our_path" ]] && ! is_ignored "$name"; then
...
# --- Section 2: Upstream changes to installed skills ---
echo ""
