...
Resolve the diff yourself with the shell. Never assume main or master as a base. When the diff covers uncommitted work, include untracked added files (`git ls-files --others --exclude-standard`). Inspect only: do not modify files or run mutating commands.
