...
Resolve the diff yourself with the shell. Inspect only: do not modify files or run mutating commands.
...
Use exact file paths as they appear in the diff header, never a bare filename. Name the affected function or method in the problem where applicable; state why it matters and provide a concrete fix when possible.
