...
const legendTitleForView = (view: BreakdownView): string =>
  Match.value(view).pipe(
    Match.when("model", () => "Top models (30d palette):"),
    Match.when("cwd", () => "Top directories (30d palette):"),
    Match.when("dow", () => "Time of day:"),
    Match.when("tod", () => "Time of day:"),
    Match.exhaustive,
  );

/** A raw JSON value as it comes out of a session file. */
type JsonValue = null | boolean | number | string | JsonValue[] | { [key: string]: JsonValue };

/**
 * Narrow a JSON value to a JSON object. Property values keep the JsonValue
 * contract, so callers can pass them straight into JsonValue-typed helpers.
 */
function asJsonRecord(value: JsonValue | undefined): { [key: string]: JsonValue } | undefined {
  // SAFETY: JsonValue's object arm is exactly { [key: string]: JsonValue };
  // isReadonlyObject verified we are in that arm, so the cast only restores
  // the field types the JSON contract already guarantees.
  return Predicate.isReadonlyObject(value) ? (value as { [key: string]: JsonValue }) : undefined;
}
...
interface BreakdownData {
  ...
  /** Files and lines the walk skipped because they were unreadable or malformed. */
  skipped: { directories: number; files: number; lines: number };
}
...
function setBorderedLoaderMessage(loader: BorderedLoader, message: string) {
  ...
  // oxlint-disable-next-line no-explicit-any -- SAFETY: BorderedLoader hides its inner
  // Loader behind a private field and the SDK exposes no accessor; the result is
  // runtime-guarded with isFunction before use.
  const inner = (loader as any)["loader"];

  if (inner && Predicate.isFunction(inner.setMessage)) {
...
/** Defect: buildRangeAgg always produces at least one day per range. */
function firstAndLastDay(range: RangeAgg) {
  const first = range.days[0];
  const last = range.days[range.days.length - 1];

  if (!first || !last) throw new Error("internal invariant: range has no days");

  return { start: first.date, end: last.date };
}

/** Defect: ranges are populated for every RANGE_DAYS entry before use. */
function rangeFor(ranges: Map<number, RangeAgg>, days: number): RangeAgg {
  const range = ranges.get(days);

  if (!range) throw new Error(`internal invariant: missing range for ${days} days`);

  return range;
}

/** Defect: rangeIndex selects within the non-empty RANGE_DAYS tuple. */
function rangeDaysForIndex(index: number): number {
  const days = RANGE_DAYS[index];

  if (!days) throw new Error(`internal invariant: no range for index ${index}`);

  return days;
}

/** Defect: mondayIndex returns 0..6, matching DOW_NAMES. */
function dowName(index: number): DowKey {
  const name = DOW_NAMES[index];

  if (!name) throw new Error(`internal invariant: weekday index ${index} out of range`);

  return name;
}

/** Defect: PALETTE is non-empty by construction. */
function paletteColor(index: number): RGB {
  const color = PALETTE[index % PALETTE.length];

  if (!color) throw new Error("internal invariant: empty palette");

  return color;
}

function modelKeyFromParts(provider?: JsonValue, model?: JsonValue): ModelKey | null {
  const p = Predicate.isString(provider) ? provider.trim() : "";
  const m = Predicate.isString(model) ? model.trim() : "";
...
function normalizedLowerString(value: JsonValue | undefined): string {
  return Predicate.isString(value) ? value.trim().toLowerCase() : "";
}

function isFauxModelReference(parts: {
  api: JsonValue | undefined;
  provider: JsonValue | undefined;
  model: JsonValue | undefined;
  modelId: JsonValue | undefined;
}): boolean {
...
function extractCostTotal(usage: JsonValue | undefined): number {
  if (!Predicate.isReadonlyObject(usage)) return 0;
  const c = usage.cost;

  if (Predicate.isNumber(c)) return Number.isFinite(c) ? c : 0;
  ...
  const t = Predicate.isReadonlyObject(c) ? c.total : undefined;

  if (Predicate.isNumber(t)) return Number.isFinite(t) ? t : 0;

  if (Predicate.isString(t)) {
    const n = Number(t);
...
function extractTokensTotal(usage: JsonValue | undefined): number {
  ...
  // - { tokens: number | { total } }
  const readNum = (v: JsonValue | undefined): number => {
    if (Predicate.isNumber(v)) return Number.isFinite(v) ? v : 0;

    if (Predicate.isString(v)) {
      const n = Number(v);
  ...
  const record = asJsonRecord(usage);
  ...
  total =
    ...
    readNum(record.tokens) ||
    readNum(record.tokenCount) ||
    readNum(record.token_count);
  ...
  const a =
    ...
    readNum(record.inputTokens) ||
    readNum(record.input_tokens);

  const b =
    readNum(record.completionTokens) ||
    readNum(record.completion_tokens) ||
    readNum(record.outputTokens) ||
    readNum(record.output_tokens);
...
async function walkSessionFiles(
...
): Promise<{ files: string[]; skippedDirectories: number; skippedFiles: number }> {
  const out: string[] = [];
  ...
  while (stack.length) {
    ...
    try {
      ...
      // Unreadable directory: count it and keep scanning.
    ...
    for (const ent of entries) {
      ...
      try {
        ...
        // Unreadable file: count it and keep scanning.
        skippedFiles += 1;
      }
    }
  }
  ...
  return { files: out, skippedDirectories, skippedFiles };
}

async function parseSessionFile(
  filePath: string,
  signal?: AbortSignal,
): Promise<{ session: ParsedSession | null; skippedLines: number }> {
  ...
  try {
    for await (const line of rl) {
      ...
      try {
        ...
        // Malformed line: count it and keep parsing the file.
        skippedLines += 1;
        continue;
      }
      ...
      if (record.type === "session") {
        if (!startedAt && Predicate.isString(record.timestamp)) {
          const d = new Date(record.timestamp);

          if (Number.isFinite(d.getTime())) startedAt = d;
        }

        if (Predicate.isString(record.cwd) && record.cwd.trim()) {
          cwd = record.cwd.trim();
        }
      ...
      if (record.type === "model_change") {
        ...
        const mk = modelKeyFromParts(record.provider, record.modelId);
        currentModel = mk;
      ...
      const { api, provider, model, modelId, usage } = extractProviderModelAndUsage(record);
  ...
  const dow = dowName(mondayIndex(startedAt));
...
function choosePaletteFromLast30Days(range30: RangeAgg, topN = 4, groupProviders = false) {
  ...
  for (const [i, key] of orderedModels.entries()) {
    modelColors.set(key, paletteColor(i));
  }
...
function chooseCwdPaletteFromLast30Days(range30: RangeAgg, topN = 4) {
  ...
  for (const [i, key] of orderedCwds.entries()) {
    cwdColors.set(key, paletteColor(i));
  }
...
function buildDowPalette() {
  ...
  // DOW_NAMES and DOW_PALETTE are the same length by construction.
  for (const [i, name] of DOW_NAMES.entries()) {
    const color = DOW_PALETTE[i];

    if (color) dowColors.set(name, color);
  }
...
/** What the metric functions report and which table column they drive. */
...
function weeksForRange(range: RangeAgg): number {
  const { start, end } = firstAndLastDay(range);
...
function renderGraphLines(
  ...
  for (let row = 0; row < 7; row++) {
    ...
    for (let w = 0; w < weeks; w++) {
      ...
      const value = Match.value(metric.kind).pipe(
        Match.when("tokens", () => day?.tokens ?? 0),
        Match.when("messages", () => day?.messages ?? 0),
        Match.when("sessions", () => day?.sessions ?? 0),
        Match.exhaustive,
      );
...
function renderCwdTable(range: RangeAgg, mode: MeasurementMode, maxRows = 8): string[] {
  ...
  for (const [i, r] of rows.entries()) {
    ...
    const costPerSession = sess > 0 ? formatUsd(cost / sess) : "-";
    const share = total > 0 ? `${Math.round((value / total) * 100)}%` : "0%";
    const displayPath = displayPaths[i] ?? r.key;
    lines.push(
      `${padRight(displayPath.slice(0, cwdWidth), cwdWidth)}  ${padLeft(formatCount(value), valueWidth)}  ${padLeft(formatUsd(cost), 10)}  ${padLeft(costPerSession, 8)}  ${padLeft(share, 6)}`,
    );
  }
...
function dowMetricForRange(range: RangeAgg, mode: MeasurementMode): DowMetric {
  const metric = graphMetricForRange(range, mode);
  const kind = metric.kind;
...
async function computeBreakdown(
  ...
  const range90 = rangeFor(ranges, 90);
  const { start: start90 } = firstAndLastDay(range90);
  ...
  const {
    ...
    skippedDirectories,
    skippedFiles,
  } = await walkSessionFiles(SESSION_ROOT, start90, signal, (found) => {
  ...
  for (const filePath of candidates) {
    ...
    const { session, skippedLines: fileSkippedLines } = await parseSessionFile(filePath, signal);
  ...
  const range30 = rangeFor(ranges, 30);
  ...
  return {
    ...
    dowPalette,
    todPalette,
    skipped: { directories: skippedDirectories, files: skippedFiles, lines: skippedLines },
  };
}
...
class BreakdownComponent implements Component {
  ...
  render(width: number): string[] {
    ...
    const selectedDays = rangeDaysForIndex(this.rangeIndex);
    const range = rangeFor(this.data.ranges, selectedDays);
    ...
    const header =
      ...
      (() => {
        ...
        if (skipped.directories + skipped.files + skipped.lines === 0) return "";

        return dim(
          `  (${skipped.directories} unreadable dirs, ${skipped.files} unreadable files, ${skipped.lines} malformed lines)`,
        );
      })();
    ...
    if (this.view === "dow") {
      ...
      const maxScale = Match.value(selectedDays).pipe(
        Match.when(7, () => 4),
        Match.when(30, () => 3),
        Match.orElse(() => 2),
      );
    ...
    const tableLines = Match.value(this.view).pipe(
      Match.when("model", () =>
        renderModelTable(range, metric.kind, maxRows, !this.splitProviders),
      ),
      Match.when("cwd", () => renderCwdTable(range, metric.kind, maxRows)),
      Match.when("dow", () => renderDowTable(range, metric.kind)),
      Match.when("tod", () => renderTodTable(range, metric.kind)),
      Match.exhaustive,
    );
    ...
    if (this.view === "dow") {
      ...
      if (showSideLegend) {
        ...
        const legendTitle = legendTitleForView(this.view);
...
/**
 * Register the interactive `session-breakdown` command: analyzes
 * ~/.pi/agent/sessions (*.jsonl) for the last 7/30/90 days and renders usage
 * by model, cwd, weekday, and time-of-day. Without a TUI it sends a summary.
 */
  ...
  pi.registerCommand("session-breakdown", {
    ...
    handler: async (_args, ctx: ExtensionContext) => {
      if (!ctx.hasUI) {
        ...
        const skippedNote =
          skipped.directories + skipped.files + skipped.lines > 0
            ? `\nNote: ${skipped.directories} unreadable dirs, ${skipped.files} unreadable files, ${skipped.lines} malformed lines skipped.`
            : "";

        pi.sendMessage(
          {
            customType: "session-breakdown",
            content: `Session breakdown (non-interactive)\n${rangeSummary(range, 30, "sessions")}${skippedNote}`,
            display: true,
          },
          { triggerTurn: false },
        );
