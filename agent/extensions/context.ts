...
import { type Component, Container, Key, matchesKey, Text, type TUI } from "@earendil-works/pi-tui";
...
function buildAnnotatedExport(snapshot: ContextSnapshot, ctx: ExtensionContext): string {
	...
	lines.push(
		`- model: ${snapshot.capturedPayload?.provider && snapshot.capturedPayload?.model ? `${snapshot.capturedPayload.provider}/${snapshot.capturedPayload.model}` : "(unknown)"}`,
	);
	lines.push("");
	...
	lines.push("");
	lines.push(
		`- extensions: ${snapshot.extensions.length ? snapshot.extensions.map((ext) => `${ext.fileName} (${ext.commands.join(", ")})`).join(", ") : "(none)"}`,
	);
	...
	if (!snapshot.tools.length) {
		...
		for (const tool of snapshot.tools) {
			lines.push(
				`- ${tool.name}: ~${tool.tokens.toLocaleString()} tok${tool.description ? ` — ${tool.description}` : ""}`,
			);
		}
		lines.push("");
	}
	...
	if (!snapshot.capturedPayload) {
		...
		lines.push(
			`- provider/model: ${snapshot.capturedPayload.provider && snapshot.capturedPayload.model ? `${snapshot.capturedPayload.provider}/${snapshot.capturedPayload.model}` : "(unknown)"}`,
		);
		lines.push("");
	...
	if (payloadBlocks.length) {
		...
		lines.push(
			"No obvious provider message array found in the captured payload. Falling back to the current session branch dump.",
		);
		lines.push("");
	...
	lines.push("");
	lines.push(
		"- The raw payload section comes from the most recent `before_provider_request` capture for this session.",
	);
	lines.push("- Token counts shown in the summary panel remain approximate, especially for tool schema overhead.");
	lines.push(
		"- If `/context` is opened before any model request in the session, raw payload export is unavailable until a request has been captured.",
	);
	lines.push("");
	...
	pi.registerCommand("context", {
		...
		handler: async (_args, ctx: ExtensionCommandContext) => {
			...
			const makePlainText = () => {
				...
				lines.push(
					`Payload: ${capturedPayload ? `captured (${formatTimestamp(capturedPayload.timestamp)})` : "no captured payload yet"}`,
				);
				return lines.join("\n");
			};
			...
			const openExport = async (kind: "raw" | "annotated", tui: TUI | undefined) => {
				if (kind === "raw" && !snapshot.capturedPayload) {
					ctx.ui.notify(
						"No captured payload yet. Run at least one model request in this session first.",
						"warning",
					);
					return;
				}
