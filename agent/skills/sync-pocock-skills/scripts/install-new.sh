...
#
# Copies the upstream skill, applies any configured patches/local overrides,
# removes from ignored.txt if previously ignored, then scans for Claude Code
# patterns that need pi patches.
...
# Remove from ignored.txt if previously ignored
IGNORED_FILE="$PATCHES_DIR/ignored.txt"
if [[ -f "$IGNORED_FILE" ]] && grep -qxF "$SKILL_NAME" "$IGNORED_FILE" 2>/dev/null; then
  grep -vxF "$SKILL_NAME" "$IGNORED_FILE" > "$IGNORED_FILE.tmp" && mv "$IGNORED_FILE.tmp" "$IGNORED_FILE"
  echo "UNIGNORED: removed $SKILL_NAME from ignored.txt"
fi

echo "INSTALLED: $SKILL_NAME"

# Only flag Claude Code-specific patterns (pi has native subagent support)
